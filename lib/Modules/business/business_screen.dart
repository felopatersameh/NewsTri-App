  ...
  Widget build(BuildContext context) {
    return BlocConsumer<CubitNews, StatesNews>(
        ...
        builder: (context, state) => ConditionalBuilder(
            condition: CubitNews.get(context).businessNews.isNotEmpty,
            builder: (context) => ListView.separated(
                  physics: const BouncingScrollPhysics(),
                  itemCount: CubitNews.get(context).businessNews.length,
                  itemBuilder: (context, index) => Articles(articles: CubitNews.get(context).businessNews[index]),
